...
actual class DataInitializer : IDataInitializer {
    ...
    companion object {
        ...
        private val appDir = File(System.getProperty("user.home"), ".purefood")
    }
    ...
    override suspend fun isDataInitialized(): Boolean = withContext(Dispatchers.IO) {
        val jsonFile = File(appDir, "recipes.json")
    ...
    private suspend fun downloadAndExtractZip(
        ...
        return@withContext try {
            ...
            try {
                ...
                val cacheDir = File(appDir, "recipe_images").apply { mkdirs() }
    ...
    private suspend fun deleteLocalData() = withContext(Dispatchers.IO) {
        File(appDir, "recipes.json").delete()
        File(appDir, "recipe_images").deleteRecursively()
    }
}
